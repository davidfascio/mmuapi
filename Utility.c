#include "Utility.h"

BYTE macLongAddrByteInverse[MAC_SIZE] = {0x24, 0xda, 0xb6, 0x0a, 0x02, 0x00, 0x06, 0x30};
BYTE macLongAddrByte[MAC_SIZE] = {0x30, 0x06, 0x00, 0x02, 0x0a ,0xb6, 0xda, 0x24};

WORD wfnCRC_CALC (BYTE *ptFRAME, WORD wSizeFrame, WORD wCRCStart){
    WORD temp;
    BYTE c, flag;
    for (wSizeFrame;wSizeFrame>0;wSizeFrame--){
        temp = (WORD) *ptFRAME;
        temp &= 0x00FF;
        wCRCStart = wCRCStart ^ temp;
        for (c=0;c<8;c++){
            flag = wCRCStart & 0x01;
            wCRCStart = wCRCStart>>1;
            if(flag != 0) wCRCStart = wCRCStart ^ 0xA001;
        }
        ptFRAME++;
    }
    //wCRCStart = (wCRCStart >> 8) | (wCRCStart << 8);
    return (wCRCStart);
}


BYTE wfnBCC_CALC (BYTE * frame, WORD frameLen, BYTE bccStart){
    
    WORD index;    
    frame += UTILITY_FRAME_BCC_START_INDEX;
    
    for(index = UTILITY_FRAME_BCC_START_INDEX; index < frameLen; index++){
     
        bccStart ^= (* frame);
        frame++;
    }
    
    return bccStart;
}

void inverted_memcpy(BYTE * data1, BYTE * data2, WORD data_size){
    
    while (data_size){
        
        data_size--;
        * data1 = *(data2 + data_size);
        data1++;
    }
}

/* Table of CRC values for highorder byte */
static const BYTE _baCRCHi[] = 
{
	
};
/* Table of CRC values for loworder byte */
static const BYTE _baCRCLo[] = 
{
	
};
//-----------------------------------------------------------------------------
/*!
	\fn		WORD wfnCRC16 ( BYTE* bpMsg, WORD wDataLen ) 
	\param	BYTE* bpMsg - Block starting address
	\param	WORD wDataLen -  Block size
	\return	CRC value
	\brief	Calculates the CRC 16 of a block of data using LUTs
*/
//-----------------------------------------------------------------------------
WORD wfnCRC16 ( BYTE* bpMsg, WORD wDataLen ) 
{
	BYTE bCRCHi = 0xFF ; /* high byte of CRC initialized */
	BYTE bCRCLo = 0xFF ; /* low byte of CRC initialized */
	WORD wIndex = 0; /* will index into CRC lookup table */
	while (wDataLen--) /* pass through message buffer */
	{
		wIndex = bCRCLo ^ *bpMsg++ ; /* calculate the CRC */
		bCRCLo = bCRCHi ^ _baCRCHi[wIndex] ;
		bCRCHi = _baCRCLo[wIndex] ;
	}
	return (bCRCHi << 8 | bCRCLo);
}